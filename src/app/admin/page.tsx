...
import { formatDate, dateKey, parseCSV } from '@/lib/crm'
...
export default function AdminPage() {
  ...
  async function unassignLeads() {
    ...
    if (!confirm(`${selectedLeads.size} Lead(s) wirklich von ihren Settern entfernen?`)) return
    for (const leadId of Array.from(selectedLeads)) {
      await fetch(`/api/leads/${leadId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  function toggleLeadCheckbox(leadId: string, index: number, shiftKey: boolean) {
    ...
    if (shiftKey && lastClickedIndex !== null) {
      ...
      const shouldSelect = !s.has(leadId)
      rangeIds.forEach(id => shouldSelect ? s.add(id) : s.delete(id))
    } else {
  ...
  async function handleImportCSV(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (parsed.length === 0) { setImportStatus('CSV leer oder ungültig'); return }
    const now = new Date().toISOString().slice(0, 10)
    const newLeads = parsed.map(l => ({ ...l, createdAt: now, assignedTo: '' }))
        ...
        {tab === 'leads' && (
          <div>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <button onClick={() => { setSelectedLeads(new Set(leads.filter(l => !l.assignedTo).map(l => l.id))); setLastClickedIndex(null) }} style={{ padding: '7px 14px', background: '#fff', border: '1px solid #fca5a5', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', color: '#dc2626', whiteSpace: 'nowrap' }}>
                Alle unzugewiesenen wählen
              </button>
            ...
            {selectedLeads.size > 0 && (
              <div style={{ background: '#eff6ff', border: '1px solid #93c5fd', borderRadius: 10, padding: '10px 16px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                ...
                <button onClick={unassignLeads} style={{ padding: '7px 18px', background: '#fff', color: '#dc2626', border: '1px solid #fca5a5', borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Zuweisung entfernen</button>
                <button onClick={() => { setSelectedLeads(new Set()); setLastClickedIndex(null) }} style={{ padding: '7px 12px', background: 'none', border: '1px solid var(--border)', borderRadius: 7, fontSize: 12, cursor: 'pointer', color: 'var(--text-muted)' }}>Abbrechen</button>
                <span style={{ fontSize: 11, color: '#93c5fd', width: '100%' }}>Tipp: Shift + Klick wählt einen ganzen Bereich aus</span>
              </div>
            )}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr style={{ background: 'var(--bg)' }}>
                    <th style={th}><input type="checkbox" onChange={e => { setSelectedLeads(e.target.checked ? new Set(filteredLeads.map(l => l.id)) : new Set()); setLastClickedIndex(null) }} checked={selectedLeads.size > 0 && selectedLeads.size === filteredLeads.length} /></th>
                    <th style={th}>Name</th>
                  ...
                  {filteredLeads.map((l, idx) => {
                    ...
                    return (
                      <tr key={l.id} style={{ borderBottom: '1px solid var(--border-light)', background: selectedLeads.has(l.id) ? 'rgba(22,163,74,0.05)' : 'transparent' }}>
                        <td style={td} onClick={e => { e.preventDefault(); toggleLeadCheckbox(l.id, idx, (e as any).shiftKey) }}>
                          <input type="checkbox" checked={selectedLeads.has(l.id)} readOnly style={{ cursor: 'pointer' }} />
                        </td>
